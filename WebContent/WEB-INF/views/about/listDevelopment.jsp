...
<body>
	<div class="topPathArea">
    	<table>
        	...
        	<td>
				<c:if test="${dto.newsTypeId == 2 }">
					发展历程
				</c:if>
				<c:if test="${dto.newsTypeId == 3 }">
					集团荣誉
				</c:if>
				<c:if test="${dto.newsTypeId == 4 }">
					产品荣誉
				</c:if>      		
            </td>
            <td>&gt;</td>
				...
				<ll:htmlTable caption="Presidents" width="95%">
					...
					<ll:htmlColumn property="selected" title="选择" editable="true" width="5%"
						...
						<ll:htmlColumn property="rowNumber" title="编号" editable="false" width="5%"
							exportable="false" />
						<ll:htmlColumn property="yearM" title="所属年份" exportable="false" styleClass="table_left_css" width="10%"/>
						<ll:htmlColumn property="monthM" title="所属月份" exportable="false" styleClass="table_left_css" width="10%"/>
